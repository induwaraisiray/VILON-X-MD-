...
async (conn, mek, m, { from, isGroup, isAdmins, reply, quoted }) => {
    try {
...
async (conn, mek, m, { from, isGroup, participants, isAdmins, reply, quoted }) => {
    try {
     // Check if the command is used in a group
     if (!isGroup) return reply("⚠️ This command can only be used in a group!");
     ...
     if (!isAdmins) return reply("⚠️ Only group admins can use this command!");
		...
		let teks = ` *GIVE YOUR ATTENTION PLEASE* ❗
...
`
        ...
        console.error("Tag All Group Error:", e);
        reply(`❌ Failed to tag group. Error: ${e.message}`);
    }        
});
...
async (conn, mek, m, { from, isGroup, participants, isAdmins, reply, quoted }) => {
    try {
     ...
     if (!isAdmins) return reply("⚠️ Only group admins can use this command!");
		...
		if(!quoted) return reply('*Please add a Message* ❗')
		let teks = `${quoted}`
                conn.sendMessage(from, { text: teks, mentions: participants.map(a => a.id) });
